...
function setup() {
  ...
  invg=createSprite(200,height-50,400,5);
  
  ground=createSprite(200,height-210,400,400);
  ground.addImage("ground",groundI);
  ground.setVelocity(-5,0);
  ground.x=ground.width/2;
  
  monkey = createSprite(70,height-60,10,10);
  ...
  h1=createSprite(20,height/2+120);
  h1.addAnimation("heart1",hp);
  h1.scale=0.06;
  
  h2 = createSprite(60,height/2+120);
  h2.addAnimation("heart2",hp);
  h2.scale=0.06;
  
  gameOver = createSprite(300,height/2,10,10); 
...
function draw() {
  ...
  monkey.setVelocity(0,-10);
      ...
      if((touches.length>0||keyDown("space")) && monkey.isTouching(upGroup)){
    monkey.setVelocity(0,-10);
  }
  ...
  if(mousePressedOver(restart)||touches.gameover){
   reset();
  ...
  fill("white");
  textSize(18);
  text("Your score:-"+score,450,height/2+130);
}
  
function spawnObstacles(){
  
  if (frameCount%200===0) {
    stone = createSprite(610, height-60,10,10);
    stone.x=610 ;
...
function spawnBanana(){
  if (frameCount%120===0) {
    banana = createSprite(610, height-120,10,10);
    banana.x=610;
    banana.y=random(height-40,height-120);
...
if (frameCount%140===0) {
    uphill = createSprite(610, height-130,10,10);
    uphill.x=610;
